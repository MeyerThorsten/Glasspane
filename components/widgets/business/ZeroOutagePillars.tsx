        ...
        return (
          <div key={pillar.name}>
            <div className="flex items-center justify-between mb-1">
              <h4 className="text-sm font-semibold text-gray-900">{pillar.name}</h4>
              <span className={`text-sm font-bold ${meetsTarget ? "text-emerald-600" : "text-amber-600"}`}>
                {pillar.score}%
                <span className="text-xs font-normal text-gray-400 ml-1">/ {pillar.target}%</span>
              </span>
            </div>
            <div className="mt-1 w-full bg-gray-100 rounded-full h-2.5 relative">
              <div
                className={`h-2.5 rounded-full ${meetsTarget ? "bg-emerald-500" : "bg-amber-500"}`}
                style={{ width: `${pillar.score}%` }}
              />
              <div
                className="absolute top-0 w-0.5 h-2.5 bg-gray-800"
              ...
              {pillar.metrics.map((metric) => (
                <div key={metric.name} className="flex items-center justify-between text-xs">
                  ...
                  <span className={`font-medium ${metric.value >= metric.target ? "text-emerald-600" : "text-amber-600"}`}>
                    ...
                    <span className="text-gray-400 font-normal ml-1">/ {metric.target} {metric.unit}</span>
                  </span>
                </div>
              ))}
            </div>
